...
\startchapter
   ...
   bookmark={A Probabilistic Framework for Goal-Oriented Requirements Engineering},
    ...
    satisfaction rate of an obstacle is defined as the highest state
    ...
    to probabilistic goals; thereby the satisfaction arguments and
    ...
    \startsection
      ...
      \startsubsection
        ...
        \startdefinition{Measurable set}
          ...
          A set is {\it measurable} if a corresponding $\sigma$-algebra exists;
          a probability measure over that $\sigma$-algebra then quantifies the
          probability of satisfying an assertion.
      ...
      \startsubsection
        ...
        Note that the probability measure is defined here over a Markov chain
        ...
        in order to make the semantics further precise.
        ...
        {\bf Water\-Pump\-On}, Water\-Pump\-Off,...)}.
        ...
        \startformula
        	Pr(Cyl({\ss WaterPumpOn,WaterPumpOff})) = 1\times.7
        \stopformula
        ...
        This section introduced the formal semantics that will be used for
        probabilistic goals and obstacles in the next section. 
      ...
      \stopsubsection
        ...
        and $I \subseteq [0,1]$ is a non-empty interval;
        ${\blackboard{}P}_I(\psi)$ is {\it true} for a state $s$ if $Pr^s(\psi)
        \in I$. The {\it behavior formulas} are built using the temporal
        operators (such as $\ltlF$, $\ltlG$, $\ltlW$, $\ltlU$) over state
        formulas. Boolean combinations of behavior formulas are not allowed in
        PCTL, but are allowed in PCTL*. PCTL* is strictly more expressive than
        PCTL. All formulas expressed in LTL can be expressed in PCTL* but not
        in PCTL \cite[Bai08].
    ...
    \startsection
      ...
      \in{Section}[ref:independence-goals] discusses the independence of goals.
        ...
        This deidealized goal states that the water level shall be above {\ss LOW}
        ...
        latter are annotated with a {\it computed} satisfaction rate and a
        {\it required} satisfaction rate.
            
        \startdefinition[dfn:esr]{Computed goal satisfaction rate}
        
          The {\it computed satisfaction rate} (CSR) of a goal is the
          satisfaction rate of this goal in view of its possible obstructions.

        \stopdefinition
          
        The CSR is computed from the goal and obstacle models;
        \in{Chapter}[chap:assessing] will describe how computed satisfaction
        rates for high-level goals are computed from the estimated satisfaction
        rate of leaf obstacles.
        ...
        desirable statisfaction rates. With such buffer zone, the ESR shall be
        above the RSR and should ideally be above the TSR. 
        ...
        Given the computed satisfaction rate $P(G)$ and the required
      ...
      \startsubsection
        ...
        exploited to syntactically determine whether two goals are weakly independent.
    ...
    \startsection
      ...
      obstacle independence.
        ...
        An obstacle $\ltlF (C \wedge \Theta OC)$ requires behaviors where at
        least a system state satisfy $C \wedge \Theta OC$. As obstacles might
        be partially satisfied, a state $s$ has a probability to satisfy $C
        \wedge \Theta OC$. Dually to goals, we take a conservative approach
        requiring an upper bound as we need to pessimistically consider the
        highest chance of goal violation.

        \startdefinition{Obstacle satisfaction rate}

          The satisfaction rate of an obstacle of form $\ltlF (C \wedge
          \Theta OC)$ is the highest state probability of the assertion $C
          \wedge \Theta OC$ in any possible state $s$.

        \stopdefinition
        
        \noindent The satisfaction rate of an obstacle $O$ is denoted by
        $P(O)$. For an obstacle of form $\ltlF (C \wedge \Theta OC)$:
        ...
        \obstacle{Pump Electrical Failure}, obtained in
        \in{Section}[sec:obstacle_analysis] and as seen in
        \in{Figure}[fig:background_make_up_water_pump_on]:
    
        \startkaosspec
            \ObstacleName {Pump Electrical Failure}
            \KaosAttribute {FormalDef} {
              $\ltlF(WaterPumpRequested \wedge \ltlG_{\geq5m} PumpElectricalFailure)$
          }
        \stopkaosspec
        ...
        \startformula
          WaterPumpRequested \wedge \ltlG_{\geq5m} PumpElectricalFailure
        \stopformula
        
        has a state probability $.9$ in $s_1$; $.8$ in $s_2$; and $.7$ in
        $s_3$. The satisfaction rate of this obstacle is the largest state
        probability, that is, $.9$. The system satisfies

        \startformula
          \ltlG~\mathbb{P}_{\leq .9} (WaterPumpRequested \wedge \ltlG_{\geq5m} PumpElectricalFailure)
        \stopformula
        ...
        In our probabilistic framework, leaf obstacles are annotated with an
        estimated satisfaction rate.
        
        \startdefinition{Estimated obstacle satisfaction rate}

          The {\it estimated satisfaction rate} (ESR) of a leaf obstacle is the
        ...
        The satisfaction rate of higher-level obstacles is computed from those
        estimates; \in{Chapter}[chap:assessing] will show how their computation
        proceeds by up-propagation through the refinement trees.
        ...
        the subjectivity in the estimates provided by the domain experts.
        ...
        \item An AND-refinement of a probabilistic obstacle is {\it complete} if the
        ...
        \stopdefinition
			  ...
			  \item An AND-refinement of a probabilistic obstacle is {\it consistent} if at
        ...
        \stopdefinition
			  ...
			  \item An AND-refinement of a probabilistic obstacle is {\it minimal} if
        ...
        \noindent This condition can be relaxed:
      ...
      \startsubsection
        ...
        In our example, the probability for a mechanical failure of the water pump to fail
        does not depend on, e.g., the probability of a leak in the spent fuel
        pool. Such assumption must be carefully checked by domain experts.
    ...
    \startsection
      ...
      As recall, the assertion $\ltlG {\blackboard{}P}_{\geq
      rsr}(C\rightarrow\Theta T)$ is satisfied by a behavior $\pi$ if all
      states $s$ along this behavior satisfy $Pr^s(C\rightarrow\Theta T) \geq
      rsr$.
   ...
   %  trivially equivalent to $True$. \stopproof
      ...
      target condition to be {\it true} (respectively {\it false}) at some point in the
      ...
      for finite violability. In M-LTL, this is formalized as:
      ...
      The probabilistic version for an {\it Achieve} goal (respectively {\it
      ...
      The probabilistic version for a {\it Maintain} goal states that the
      target condition must hold with at least some given probability threshold
      $p$. For a probabilistic {\it Avoid} goal, the target condition may not
      hold with at least a probability threshold $p$. This can be formalized as
  ...
  \startsection[title={Summary}]
    ...
    together with a precise definition in terms of states and behaviors. This
    ...
    probabilities. The chapter provided precise characterization of the
    independence for probabilistic goals and obstacles. It showed how such
    characterization might be enforced by leveraging the refinement structure.
    The chapter also showed how the specification of probabilistic goals may be
    facilitated by use of specification patterns to enable formal analysis.
    ...
    enables the monitoring, at system runtime, of probabilistic obstacles for
