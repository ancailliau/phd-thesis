...
\chapter[chap:relatedwork]{Related Work}
  ...
  \cite[McD99], or {\it Misuse Case} \cite[Sin05]. These frameworks deals
  ...
  \startsection[reference=sec:comparing_ra,title={Risk Analysis}]
    ...
    comprehensive introduction to FTA is provided in \cite[Ves81]. We briefly discuss here classical fault
    ...
    \cite[Ear92,Chu93,Bur93,Fen94,McD94,McD95,McD02,Par07].
    ...
    developed for software systems \cite[Bur93,McD94,McD95,McD02,Han04].
    ...
    criticality matrix helps the risk engineers to identify likely and critical 
    ...
    risk analysis \cite[Ref15]. Likelihoods may be specified
    using equations combining key indicators whose value might change over
    time \cite[Ref09]. However, little support is provided to identify such key indicators,
    ...
    and columns are the consequences, to identify likely and critical  risks.
  ...
  \startsection[reference=sec:comparing_qre,title={Quantitative Requirements Engineering}]
    ...
    \startitemize
      ...
      \item Unlike \cite[Gio02, Gio03, Gio05, Fra03, Fra04, Amy10,
      Cha13], our approach compares the {\it computed} satisfaction rate to
      ...
      Amy10, Cha13], goals are estimated with
  ...
  \startsection[reference=sec:comparing_rdre,title={Risk-Driven Requirements Engineering}]
    ...
    \noindent {\bf Goal-Risk Framework.} \cite[Asn06,Asn11] extends the Tropos
    framework with risks and countermeasures. In the framework, a {\it risk} is
    an event having an adverse impact on goals whereas a {\it countermeasure}
    is a treatment that can be adopted to mitigate the effect of the risk. The
    ...
    event layer} and a {\it treatment layer} with {\it impact} links between
    events and goals, and {\it effect} links between treatments and events. An
    event is annotated with a (risk) likelihood, and the (risk) severity is
    modeled as a contribution (positive or negative) from an event to goal.
    Events and treatments can be AND-/OR-refined and can contribute to other
    goals, events, or treatments. Different strategies are available for
    identifying possible treatments \cite[Asn06b]. The framework proposed in
    \cite[Asn06] is extended to support treatments mitigating the impact of an
    event \cite[Asn06], trust between the actors of the system
    \cite[Asn07], and quantitative reasoning \cite[Asn07c].
    ...
    accepted) for the top goals. Using backward analysis \cite[Hor10,Seb04], it
    identifies the leaf goals that are sufficient to satisfy the root goals. It
    then assess, using forward analysis \cite[Gio05] (i.e. propagates the
    ...
    guarantee these event values. Such analysis may also be performed on
    quantitative inputs \cite[Asn07c].
    ...
    the degree of belief of higher-level goals to be be computed from the
    ...
    \startitemize
      ...
      \item Unlike \cite[Fea03,Asn11,Sab11,Sha12,Sha13,Sie14], our
      probabilistic goals are supported in terms of {\it computed} vs. {\it
  ...
  \startsection[reference=sec:comparing_um,title={Requirements Uncertainty Managment}]
    ...
    modeling, based on obstacles, identify the sources of uncertainty in the
    satisfaction of the goals \cite[Che09c]. The countermeasures either changes
    the goal model either RELAXes the goal specifications. In
    ...
    function quantifies the degree of goal satisfaction. A fuzzy logic-based
    approach combines both crisp and fuzzy goals \cite[Cha16,Cha16b]. A
    bottom-up propagation algorithm computes the degree of satisfaction for
  ...
  \startsection[reference=sec:comparing_cr,title={Models and Techniques for Controlling Risks}]
    ...
    Alternative designs that minimize the risk of some specified high-level
  ...
  \startsection[reference=sec:comparing_as,title={Requirements-Driven Runtime Adaptation}]
    ...
    \cite[Che09b,Saw10,DeL13,Yan14,Jur15] for an overview of the
    ...
    probabilities for the leaf goals. The impact of context on the failure
    probabilities is briefly discussed in \cite[Men14].
    ...
    In \cite[Fil11,Fil14,Fil15b], a control-theoretical approach automatically
    generates controllers for self-adaptive systems. The user provides a
    running software, a parameter that impacts some non-functional
    ...
    runtime, using {\it ReqMon}. {\it Evolution Requirements} ({\it EvoReq})
    enables reasoning about the evolution of requirements \cite[Sou12]. {\it
    Evolution Requirements} are meta-requirements contraining how system
    ...
    feedback loop for self-adaptive systems \cite[Sou13]. {\it AwReq} indicates
    the situations in which an adaptation is required whereas {\it EvoReq}
    prescribes what adaptation shall be deployed. How awareness requirements
    interacts in a control-theoritic approach for self-adaptive systems is
    discussed in \cite[Sou11].
    ...
    choose the best configuration for an overlay network. Sequences of actions
    to change a software system from one configuration to another may then be
    generated \cite[Ram10]. In \cite[Ram11b] utility functions are generated
    from a goal model with RELAXed requirements to enable the monitoring of
    goals. Depending on the type of goal, the technique automatically generates
    a state-based (invariant goal), metric-based (real-valued goal) or fuzzy
    logic-based (RELAX goal) utility function that, fed with monitored values,
    provides an estimate for the goal satisfaction. Utility scores are then
    propagated in the goal model from the monitored goals to the root goals.
    \cite[Che13] illustrates how these techniques intertwine to enable
    self-adaptation of software systems.
    ...
    \startitemize
      ...
      guaranteeing that the computed satisfaction rate is above the required
    ...
    \cite[Gar03,Gar04,Gar09], focuses on architectural models. These approaches
    tend to focus on low-level requirements and do not benefit from the
    refinement structure enabling more complex adaptations; A detailed
    comparison between the requirements-based approach Zanshin and the
    architecture-based approach Rainbow \cite[Gar04] is available in
    \cite[Ang13]. Other approaches, such as \cite[Che14,Kra07], mixes both
    requirements and architectural model to propose better adaptations
    \cite[Saw10,Nus01]. Other approaches such as \cite[Cas11] tackles the
    different problem of localizing faulty components by monitoring a software
    system.
