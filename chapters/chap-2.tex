...
\chapter
  ...
  for anticipating what could go wrong.
    ...
    \subsection{Goals, requirements, and environment expectations}
      ...
      For example, a goal in a nuclear spent fuel pool may express that makeup
      water is provided when there is a loss of cooling in the pool:

      \startkaosspec
          \GoalName {Make Up Water Provided When Loss Of Cooling}
          \KaosAttribute {Definition} {
            Cold makeup water shall be provided when the pool is not cooled
            appropriately.
        }
      \stopkaosspec
      ...
      this section). The above goal is named \goal{MakeUp Water Provided When
      Loss Of Cooling}.
    ...
    \subsection {Domain properties and domain hypotheses}
      ...
      considered valid in the system-to-be. As seen later, domain properties are often 
      involved in refinements of goals and obstacles. An example of domain property in
      our running example is the fact that an electrical pump cannot operate
      ...
      domain properties as they are not necessarily always valid in the system\emdash{}but are
      assumed, by the analyst, to be true. For example, the fact that a pump
      might not be turned on when its internal electrical relay is broken is capture
      as a domain hypothesis:
    ...
    \subsection{Goal conflicts}
      ...
      \in{Chapter}[chap:assessing] and \in{}[chap:controlling_obstacle] are
      ...
      The condition $B \wedge Dom$ has to be realizable by the environment. The
      latter has to monitor all the variables constrained by the goals and
      control all the other ones \cite[DeL07].
    ...
    \subsection{Soft goals as optimisation criteria}
      ...
      are referred as {\it optimization goals}.
  ...
  \stopsection
    ...
    \subsection {What are Obstacles?}
      ...
      As a {\it consequence} of an obtacle occuring, some goals and ancestor
    ...
    \subsection {Specifying obstacles in Metric Linear Temporal Logic}
      ...
      \wedge \Theta OC)$ is often used where $OC$ is the {\it obstacle
      condition}.
    ...
    \startsubsection[title={Modelling Obstacles as AND/OR Graph}]
      ...
      AND-refinements are ideally disjoint. This condition is usefull for
      ...
      The subobstacle $SO_i$ in an OR-refinement must entail the parent
      obstacle:
    ...
    countermeasure goals. Let us briefly detail the obstacle analysis steps.
    ...
    selected countermeasures are then deployed at RE time deferred until
    runtime.
    ...
    [Make Up Water Provided When Requested]} is guaranteed even when the
    primary pump fails. In this specific case, the mitigation does not require
    a weaking of the ancestor goal.
