...
\startchapter[reference=chap:tool_support,title={Tool support}]
  ...
  its source code enabling other researchers to easily reuse or extend the tool
  suite. The tool suite accepts a textual declaration of goal and obstacle
  models that can be easily shared and integrated with existing versionning
  tools. The proposed tool suite was designed to run on remote servers through
  command-line tools.
  ...
  its corresponding obstacle model. This section presents the declaration
  language, as a textual format, used for input to the tools.
  ...
  {\it Goal refinements} are declared using the keyword
  \typeKAOSTool{refinedby}. The list of identifiers refers to other goals,
  domain properties or domain hypotheses in the corresponding AND-Refinement.
  If an identifier is not explicitly defined, a goal with corresponding
  identifier is added to the model. For example, \in{Code}[code:goal_1]
  specifies a single AND-Refinement with two subgoals, namely,
  \typeKAOSTool{response_to_loss_of_} \typeKAOSTool{cooling} and
  ...
  \typeKAOSTool{amb_mobilized_when_amb_allocated_on_} \typeKAOSTool{road} satisfy its parent
...
declare obstacle [ manual_opening_valve_failure ]
  ...
  \placetable[bottom][tab:multisatrate]
...
%  \bTR \bTD \typeKAOSTool{mitigation}         \eTD \bTD Mitigation (strong or weak) \eTD \eTR
  ...
  how the \agent{Operator} agent is assigned to the goal \goal{Achieve [Valve
  ...
  As seen in \in{Code}[code:pred_1], predicates may have arguments. In the
  example, the formal specification states that the predicate is true if the
  attribute \typeKAOSTool{Started} is true for the instance of
  \typeKAOSTool{DieselGenerator} provided as an argument. When referring to a
  predicate in a formal specification, the actual arguments must match the
  order and the respective entity types declared in the argument list.
  ...
  type is a base type, such as {\it Boolean} or {\it String}, or a
  domain-specific type such as {\it MotorStatus}. The latter are declared using
  the \typeKAOSTool{type} keyword. \in{Code}[code:object_1] shows an example of
  ...
  For a calibration variable, the reference value is declared using the
  \typeKAOSTool{esr} attribute without declaring an expert. The estimates by
  experts are declared using the \typeKAOSTool{esr} attribute with the
  corresponding expert between square brackets. When declaring the estimated
  satisfaction rate by an expert in an obstacle, the expert is also declared in
  square brackets (see \in{Code}[code:expert_2]).
  ...
  custom attributes, split the model into different files, and declare model
  managment attributes such as the title of the model, the version, and the
  authors. Utilities are also available to help analysts writing declarations.
  ...
  \startsection[reference=sec:kt_arch,title={KAOSTools: Architecture}]
    ...
    \tool{BDDSharp} is a freely available library to create and manipulate
    ...
    \tool{ExpertOpinionSharp} to combine expert opinions. The latter was
    developed specifically for combining expert estimates using calibration
    variables as a separate library \cite[Cai17e]; other approaches can thereby
    reuse these combination techniques.
  ...
  \startsection[reference=sec:kt_tools,title={KAOSTools: Tool suite}]
    ...
    \noindent {\bf Utils.ViolationDiagram.} This tool generates the data for
    violation diagrams and violation diagrams with uncertainty. The latter ones can
    then be generated using R \cite[Tea00].
  ...
  \startsection[title={Summary}]
    ...
    This chapter briefly presented the tool suite supporting the techniques
