...
declare obstacle [ manual_opening_valve_failure ]
  ...
  \bTR \bTD \typeKAOSTool{beta}         \eTD \bTD Beta Distribution        \eTD \bTD \typeKAOSTool{beta[2,4]}. A beta distribution with $\alpha = 2$ and $\beta = 4$. See \cite[Vos08a] for details on the parameters $\alpha$ and $\beta$. \eTD \eTR
  ...
  \bTR \bTD \typeKAOSTool{pert}         \eTD \bTD PERT Distribution        \eTD \bTD \typeKAOSTool{pert[.1,.4,.6]}. A PERT distribution with a $.1$ min-value, $.6$ max-value and a $.4$ mode. See \cite[Vos08a] for details about the PERT distribution. \eTD \eTR
...
%  \bTR \bTD \typeKAOSTool{mitigation}         \eTD \bTD Mitigation (strong or weak) \eTD \eTR
  ...
  multiplicities \cite[Lam09a]. \in{Code}[code:object_2] provides an example
  ...
  \cite[Ols10a]. This tool was used to produce all goal and obstacles diagrams
  presented in the thesis. Last, a bundle is available for the TextMate text
  editor \cite[Gra07a] to provide syntax highlighting.
  ...
  \startsection[reference=sec:kt_arch,title={KAOSTools Architecture}]
    ...
    outside the framework. Arrows indicates {\it USE} links \cite[Med10a].
  ...
  \startsection[reference=sec:kt_tools,title={KAOSTools Tool Suite}]
    ...
    can then be generated using R \cite[Tea00a].
    ...
    * [OptimalSelection: Resolutions={redundant_pump_started}, 
    ...
    RabbitMQ queue \cite[Vid12a]. \in{Section}[sec:bads] provides more details
