...
\chapter[chap:conlusion]{Conclusion}
  ...
  \section {Summary of contributions}
    ...
    assessment} and {\it control} steps by proposing a quantitative
    framework for probabilistic goals and obstacles.

    For the {\it obstacle assessment} step, the thesis proposed a simple yet
    effective quantitative model-based technique, anchored on the existing
    goal-oriented KAOS framework for requirements engineering. The framework is
    extended with a probabilistic layer allowing behavioral goals to be
    specified, as a first-class citizen, in terms of their required and
    estimated satisfaction rate. The thesis generalizes goals to probabilistic
    goals, together with the corresponding refinement structure and
    satisfaction arguments.
    
    The satisfaction rate of non-leaf obstacles is determined by up-propagation
    in the obstacle trees. The criticality of obstacles is systematically
    ...
    The most likely and critical obstacle combinations are then highlighted to
    guide the exploration of countermeasures in the next {\it control} step.
    
    Regarding the {\it obstacle control} step, the thesis proposed to select
    appropriate countermeasures guaranteeing the satisfaction of the required
    satisfaction rate of the high-level goals while minimizing the cost of
    resolution. An integration mechanism was introduced as a model
    ...
    the ideal goal model with dedicated constructs. Refactoring operators
    allows the analyst to attach and detach exceptions.
    ...
    criticality of obstacles is quantified via two metrics: goal violation
    uncertainty and uncertainty spread. The satisfaction rate of non-leaf
    obstacles and the satisfaction rate of goals, together with their
    ...
    uncertainty margins are highlighted. Problematic uncertainties may then
    be reduced by combining multiple sources of estimates. Countermeasures may
    be selected to guarantee the required satisfaction rate of high-level goals
    up to an uncertainty threshold.
  ...
  \section {Strenghts and limitations}
    ...
    \startitemize
      ...
      together with non-probabilistic ones in the same requirement engineering
      framework.
      ...
      non-leaf obstacles is systematically determined by up-propagation through
      the obstacle refinement trees, from the estimated satisfaction rates for
      leaf obstacles. Leaf obstacles only need to be estimated by domain
      experts.
      ...
      obstacle satisfaction rate throught the goal refinement trees. The
      criticality of an obstacle is then determined as the loss in satisfaction
      for these obstructed goals.
      ...
      high-level goals is maximized while minimizing the pertaining cost.
      ...
      the goal model intact. Normal behaviors, i.e., not impacted by the
      ...
      models, as seen in \in{Section}[sec:propagation], guarantee that the
      refinement structure remains complete, consistent and minimal.
      ...
      the constructs introduced in \in{Section}[sec:exception_handling], the
      original specification of the anchor goal and its refinement structure
      are preserved. The {\it Except} and {\it Provided} constructs encapsulate
      the modifications for a more robust system.
      ...
      their uncertainty margin about estimates.
      ...
      introduced for measuring problematic uncertainties. When applied to the
      probability distributions obtained by up-propagation to the top-level
      goals, the metrics allow critical leaf obstacles with most problematic
      ...
      Estimates are elicited from multiple sources and combined in order to
      reduce their uncertainty margins.
    
      \item {\it Satisfaction rate of probabilistic goals and obstacles may be
      monitored at runtime.} Monitored satisfaction rates are defined precisely
      in terms of observed states and behaviors. Our monitoring technique
      extends the $LTL_3$ approach \cite[Bau11] to support monitoring of
      probabilistic assertions. The monitors are built at RE time from the
      formal specification of leaf obstacles; at runtime, virtual copies of
      $LTL_3$ monitors keep track of obstacle satisfaction. State probabilities
      for each observed state can thereby be obtained to yield satisfaction
      rates for the monitored obstacles. The latter are propagated through the
      obstacle/goal model up to the system’s high-level goals.
      ...
      parallelism is often quite challenging. Unlike \cite[Ghe09,Ghe13], our
      monitoring technique does not requires the Buchï automaton to be combined
      to a behavior model in order to obtain parametric expressions to be
      monitored at runtime.
    
      \item {\it Probabilistic goals and obstacles ensures that only required
      adaptation are performed.} The risk-driven runtime adaptation approach
      ensures that the required satisfaction rate of high-level goals remains
      satisfied when the obstacle satisfaction rates are changing; adaptation
      is only required when the required satisfaction rate is not met.
    
      \item {\it Probabilistic goals and obstacles support traceability
      managment.} The predicates to be monitored are infered from the formal
      specification of the leaf obstacles to be monitored; why such or such
      monitored information is required is thereby documented. Decision
      criteria for countermeasure selection, and therefore system adaptation,
      are traceable to system objectives. Lastly, exception handling constructs
      such as {\it Except}, {\it Provided}, and {\it RelaxedTo} document why
      goal revisions were required.
    ...
    \startitemize
      ...
      addition, a resolution tactic transfer the responsibilities of an
      unrealiable to a more reliable agent. How agent reliability can be
      precisely defined at the requirements level, how it impacts the obstacle
      assessment and control are overlooked in the thesis.
      ...
      Dynamic Fault Trees \cite[Rui15], the proposed techniques currently
      overlook such system dynamic.
        
      \item {\it The risk-driven runtime adaptation technique makes simplistic
      hypothesis.} The process of runtime adaptation should take into account,
      among others, the timing issues such as the time to deploy a
      countermeasure or when an adaptation shall be executed, or the transient
      aspects of adaptation such as stability. The proposed technique for
      risk-driven runtime adaptation ignores such complexity.
  ...
  assessment and control steps are as good as the initial obstacle
  ...
  \noindent {\bf Dynamic risk analysis.} Most of the analyses are performed on
  snapshots of the software system, where the dynamic of the system and its
  evolution over time are barely taken into account. Techniques for Dynamic
  Fault Trees \cite[Rui15] may improve the quality of the probabilistic goal
  and obstacle models. How to model, elicit, and analyse a high level of
  interdependency between probabilistic goals and obstacles is an open question
  to us.
  
  \noindent {\bf Risk-driven Adaptation without monitoring.} The risk-driven
  adaptation technique relies on satisfaction rates monitoring. However,
  ...
  abrupt changes such as \cite[Epi10,Liu13,Fil14] and the filtering approach
  proposed in \cite[Fil15] could improve the accuracy of the monitored
  satisfaction rates.
  ...
  should be taken into account at RE time \cite[Cam14,Cam16]. Prediction
  approaches, such as \cite[Pol07,Mor16], appears to be a promising direction
  ...
  ignoring the above situation. Two interesting directions might improve the
  obstacle assessment and control step.
  ...
  \cite[McD99a], or {\it Misuse Case} \cite[Sin05] could exhibit interesting
