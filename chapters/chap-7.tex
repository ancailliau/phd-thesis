...
\chapter[runtime]{Handling Obstacles At System Runtime}
  ...
  \startsection[reference=sec:overview, title={Overview of the approach}]
    ...
    \startitemize[n]
      ...
      \item {\bf Updating Obstacle Monitors.} At runtime, the states of the
      monitored system are observed at a regular pace. (The pace may be chosen
  ...
  \startsection
    ...
    \startsubsection
      ...
      and not satisfying $C \wedge \Theta OC$ from states $s$; the ratio
      between the number of observed behaviors satisfying the formula and the
      number of observed behaviors estimates the corresponding state
      ...
      \startitemize[a]
        ...
        \item (Optional) Observed states are filtered;
        
        \item Monitors are created and updated according the observed states;
        
        \item Monitors labeled with $T$ (true) and $F$ (false) are counted; The
        ratio between the number of $T$ (true) and $F$ (false) monitors
        provides the monitored satisfaction rate.
      
      \stopitemize
      
      \noindent {\bf Observing states.} Consider the obstacle \obstacle{Dusty
      ...
      \noindent {\bf Filtering observed states.} As the time goes by, the
      number of monitors increases and the monitored state probability will
      ...
      might strongly impact the state probability, e.g., from $\vfrac{5}{10} =
      .5$ to $\vfrac{15}{20} = .75$ whereas when 1000 observations have been
      made, the state probability would only chage from $\vfrac{5}{1000} =
      .005$ to $\vfrac{15}{1010} = 0.014$.
      
      If the \quote{exact} satisfaction rate does not vary a lot, this is not a
      problem. On the contrary, if the \quote{real} satisfaction rate does
      change over time, this will smooth out the variations.
      
      To circumvent that effet, the process for obtaining the monitored
      satisfaction rate is modified to forget old information \cite[Cal11].
      ...
      adequatly depending on the domain, how fast states are observed, and the
      desired innaccuracy margins (see \in{Section}[sec:innaccuracy_margins]).
      Such appropriate number might also be automatically set to an optimal
      aging factor \cite[Cal14].
      
      \noindent {\bf Creating and updating monitors.} For a monitored leaf
      ...
      Other monitoring techniques such as \cite[dAm05a, Gia01, Hav04, Tha05]
      might be used to determine the satisfaction of $C\wedge\Theta OC$.
      $LTL_3$, however, reports both violation and satisfaction of the formula
      ...
      as \cite[dAm05a] amalgamate the last two cases. Other monitoring
      ...
      \noindent {\bf Obtaining satisfaction rate.} In this setting, the {\it
      monitored state probability} of state $s$ is the ratio between the number
      of monitors started in $s$ whose current state is labeled as $T$ (true),
      and the number of monitors started in $s$ whose current state is labeled
      ...
      monitors is kept in memory for each observed state, and the number of
  ...
  \stopsection
    ...
    \startitemize[a]
      ...
      \sym{\it(e*)} A Beta probability distribution is fitted to the number of
      monitors;
    
      \sym{\it(f*)} The Beta distribution updates the current monitored
      satisfaction rate using Bayesian inference; The updated satisfaction rate
      becomes the new current satisfaction rate.
    
    \stopitemize
    
    The step {\it (e)} is straightforward as the parameters of the Beta
    distribution are, $\alpha$, the number of positive monitors and, $\beta$,
    the number of negative monitors \cite[Vos08].
    
    The computation in step {\it (f)} relies on Bayesian Inference. Bayesian
    Inference is a statistical computation in which the Bayes' theorem is used
    to update a probability distribution as more information becomes available
    \cite[Wac07]. The updated satisfaction rate $P(O\mid M)$ ($M$ represents
    what has has been observed last) is given by
