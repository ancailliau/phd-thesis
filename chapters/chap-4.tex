...
\chapter
  ...
  \startsection
    [reference=sec:indentifying_most_critical_obstacles,
     title={Identifying most likely and critical obstacles}]
    ...
    An obstacle is said to be {\it tolerable} if the goal's computed
    satisfaction rate falls below the tolerable satisfaction rate but above the
    ...
    \noindent To evaluate obstacle criticality, we may proceed in two ways:
    ...
    impact analysis to global impact analysis. A set of obstacle is {\it
    critical} if the obstacles together causes the violation severity to drop
    below $0$.
    ...
    \startdefinition{Most critical obstacle}
      ...
      A probabilistic obstacle $O$ is a {\it most critical obstacle} for a
      goal $G$ if obstacle $O$ is critical and no other obstacle results in a
      higher violation severity $SV(G)$.
  
    \stopdefinition
    
     A set of obstacle is {\it a most critical set of obstacles} if no other
     set of obstacles of the same size causes a higher violation severity.
  ...
  % assigned to the root goal $G$.
    ...
    contains a single leaf obstacle. We can see that most of the obstacles
    taken alone results in a violation severity equal to 0; these are therefore
    not critical for the system. Three obstacles are however critical, namely
    \obstacle{Pump Mechanical Failure}, \obstacle{UPS Battery Failure}, and
    \obstacle{Power Cabling Failure}. These critical obstacles should be
    ...
    \indentation Criticality assessment should be an iterative process. Once
    most likely and critical obstacle combinations of size $i$ are resolved, a
    new violation diagram should be produced with obstacle combinations of size
    ...
    \in{Figure}[fig:violation_diagram_single_value]. Therefore, there are 3
    pairs of leaf obstacles that are critical and do not contain an obstacle
    critical alone. Those are (\obstacle{Pump Electrical Failure} and
    \obstacle{Alarm Not Raised And Low Water}), (\obstacle{Pump Electrical
    Failure}, \obstacle{Valve Mechanical Failure}), and (\obstacle{Pump
    Electrical Failure}, \obstacle{Valve Electrical Failure}). In our
    experience, we observed that most of the likely and critical obstacle
    combinations are caused only by a subset of these.
